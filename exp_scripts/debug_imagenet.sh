...
if [ "$debug" = "true" ]; then
    ...
    torchrun --nproc_per_node=8 -m cosmos_predict1.tokenizer.training.train  --config=cosmos_predict1/tokenizer/training/configs/config.py --     experiment=ADV8x16x16_256p_ImageNet_Posttrain job.name=VIT_for_imagenet_1x8x8_${timestamp}
fi
